...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <section className="animate-fade-up flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="font-display text-5xl font-extrabold tracking-tight text-white sm:text-6xl">
            BizChat
          </p>
          <h1 className="mt-3 font-display text-xl font-semibold tracking-tight text-canary sm:text-2xl">
            {business?.name || "Kalendarz"}
          </h1>
          <p className="mt-2 max-w-xl text-sm text-[var(--muted)]">
            Harmonogram wizyt · {business?.timezone || "Europe/Warsaw"}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Link to="/appointments">
            <GlassButton variant="subtle">+ Nowa wizyta</GlassButton>
          </Link>
        </div>
      </section>
      ...
      <div
        className="animate-fade-up grid gap-4 lg:grid-cols-[1fr_280px]"
        ...
        <aside className="space-y-4">
          <GlassCard>
            ...
            {summary.alerts_open > 0 && (
              <Link
                to="/feedback"
                className="mt-3 inline-block text-xs text-red-300"
              >
...
function AnalyticsStrip({
  ...
  const maxBar = Math.max(
    1,
    ...analytics.days.map(
      (d) => d.confirmed + d.cancelled + d.no_show + d.completed,
    ),
  );
    ...
    >
      <GlassCard>
        <div className="mb-4 flex items-end justify-between">
          <div>
            <p className="font-display text-lg font-semibold">Wizyty · 7 dni</p>
            <p className="text-xs text-[var(--muted)]">
              potwierdzone / anulacje / no-show
            </p>
          </div>
          <p className="text-xs text-[var(--muted)]">
        ...
        <div className="flex h-36 items-end gap-1.5">
          {analytics.days.map((d) => {
            const total =
              d.confirmed + d.cancelled + d.no_show + d.completed || 0.15;
            ...
            return (
              ...
              >
                <div
                  className="relative flex w-full flex-col-reverse overflow-hidden rounded-t-md border border-glass-border"
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      height: `${(d.completed / total) * 100}%`,
                      minHeight: d.completed ? 2 : 0,
                    }}
                  />
                  <div
                    className="w-full bg-orange-400/70"
                    style={{
                      height: `${(d.cancelled / total) * 100}%`,
                      minHeight: d.cancelled ? 2 : 0,
                    }}
                  />
                  <div
                    className="w-full bg-red-400/80"
                    style={{
                      height: `${(d.no_show / total) * 100}%`,
                      minHeight: d.no_show ? 2 : 0,
                    }}
                  />
                </div>
      ...
      <GlassCard>
        <p className="font-display text-lg font-semibold">Źródła rezerwacji</p>
        <p className="mb-4 text-xs text-[var(--muted)]">
          kanały · ostatnie 7 dni
          {analytics.feedback_avg != null
            ? ` · średnia opinia ★ ${analytics.feedback_avg}`
            : ""}
        </p>
        <ul className="space-y-2">
          {analytics.by_channel.slice(0, 6).map((ch) => (
            <li key={ch.channel} className="flex items-center gap-3 text-sm">
              <span className="w-24 shrink-0 capitalize text-[var(--muted)]">
                {ch.channel}
              </span>
              ...
              <span className="w-6 text-right font-display text-xs">
                {ch.count}
              </span>
            </li>
          ))}
    ...
    <>
      ...
      {Array.from({ length: dayCount }, (_, dayIndex) => {
        ...
        return (
          <div
            key={`${hour}-${dayIndex}`}
            className="relative min-h-[72px] border-b border-l border-glass-border bg-white/[0.015] p-1.5"
          >
